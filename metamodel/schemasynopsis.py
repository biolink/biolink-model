...
from metamodel.builtins import builtin_names
from metamodel.metamodel import SchemaDefinition, Element, Definition
from metamodel.metamodelcore import empty_dict, empty_set
...
class SchemaSynopsis:
    ...
    roots: References = References()            # Entities with no isa's
    mixins: References = References()           # Entities declared as mixin
    abstracts: References = References()        # Entities declared as abstract
    ...
    def eval_schema(self, schema: SchemaDefinition) -> "SchemaSynopsis":
        # Domains and ranges
        for k, v in schema.slots.items():
            ...
            self.add_ref(SlotType, k, ClassType if rng in schema.classes else TypeType, rng)
            if v.inverse:
    ...
    def errors(self) -> List[str]:
        ...
        if undefined_slots:
            rval += [f"\tUndefined slots: {', '.join(undefined_classes)}"]
        return rval
