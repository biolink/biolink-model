...
class SchemaLoader:
    ...
    def resolve(self) -> SchemaDefinition:
        ...
        for cls in self.schema.classes.values():
            for slot_name, slot_usage in cls.slot_usage.items():
                ...
                if parent_slot is not None:
                    ...
                    print(f"*** {cls.name}: no parent {slot_name}")
        ...
        #
        # Check for errors
        errors = SchemaSynopsis().eval_schema(self.schema).errors()
        if errors:
            print('\nERROR -> '.join(errors))
